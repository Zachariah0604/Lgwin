...
public class PreNextHandler : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        caption = mod.Caption;
        list = conbll.GetList(1, 1, "id<'" + id + "' and [caption]='" + caption + "' and auditing=1", out count);
        if (list.Count > 0)
        {
            if(list[0].Url=="")
               preone = "<a href='" + list[0].Id + ".html' title='" + list[0].Title + "' >" + Mystring.lim_withoutPoint(list[0].Title, 40) + "</a>";
            else
               preone = "<a href='" + list[0].Url+ "' title='" + list[0].Title + "' >" + Mystring.lim_withoutPoint(list[0].Title, 40) + "</a>";
        }
        else
            preone = "没有了！";
        list = conbll.GetList(1, 1, "id>'" + id + "' and [caption]='" + caption + "' and auditing=1", "id", false, out count);//取下一篇时要正序取
        if (list.Count > 0)
        {
            if(list[0].Url!="")
            nextone = "<a href='" + list[0].Id + ".html' title='" + list[0].Title + "' >" + Mystring.lim_withoutPoint(list[0].Title, 40) + "</a>";
            else
            nextone = "<a href='" + list[0].Url + "' title='" + list[0].Title + "' >" + Mystring.lim_withoutPoint(list[0].Title, 40) + "</a>";
        }
        else
            nextone = "没有了！";
