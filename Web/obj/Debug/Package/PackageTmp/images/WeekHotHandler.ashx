    ...
    public void ProcessRequest (HttpContext context) {
        ...
        if (con >= 10)
        {
            for (int i = 0; i < 10; i++)
            {
                ...
                target = "<tr><td class='NewsList' >" + ToHtml.GetHref_danyinhao(list[i], 12) + "(" + list[i].Counter + ")</td></tr>";
        ...
        else
        {
            for (int i = 0; i < con; i++)
            {
                target = "<tr><td class='NewsList' >" + ToHtml.GetHref_danyinhao(list[i], 12) + "(" + list[i].Counter + ")</td>";
                //target = "<tr><td class='NewsList' >" + ToHtml.GetHref_danyinhao(list[i], 10) + "(" + list[i].Counter + ")</td><td class='time'>[" + list[i].Datee.ToShortDateString() + "]</td></tr>";
                HttpContext.Current.Response.Write("document.write(\"" + target + "\");");
            } 
        }
