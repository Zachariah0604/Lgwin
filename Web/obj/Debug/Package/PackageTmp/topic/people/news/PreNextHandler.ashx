...
public class PreNextHandler : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        if (list.Count > 0)
            preone = "<a href='" + list[0].Id + ".html' title='" + list[0].Title + "' >" + Mystring.lim_withoutPoint(list[0].Title, 18) + "</a>";
        else
            preone = "没有了！";
        list = conbll.GetList(1, 1, "id>'" + id + "' and [zt]='" + zt + "' and [ztid]='" + ztlm + "' and auditing=1", "id", false, out count);//取下一篇时要正序取
        if (list.Count > 0)
            nextone = "<a href='" + list[0].Id + ".html' title='" + list[0].Title + "' >" + Mystring.lim_withoutPoint(list[0].Title, 18) + "</a>";
        else
            nextone = "没有了！";
