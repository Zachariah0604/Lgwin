package {
	...
	public class Main extends Sprite {
		...
		private function oncomplete(evt:Event) {
			...
			_progressbar.y=158;
			//*****************进度条背景的初始化位置
			_progressbackground.x=85;
			_progressbackground.y=158;
			//*****************进度滑块的初始化位置
			_progressbutton.x=85;
			_progressbutton.y=156.5;
			//*****************播放暂停按钮的初始化位置
			_playpausebutton.x=5;
			_playpausebutton.y=150;
			//*****************停止按钮的初始化位置
			_stopbutton.x=40;
			_stopbutton.y=150;
			//*****************背景的初始化位置
			...
			_background.y=110;
			//*****************播放进度条的初始化位置
			_playprogressbar.x=85;
			_playprogressbar.y=158;
			//*****************音量控制按钮的初始化位置
			_soundbutton.x=620;
			_soundbutton.y=150;
		...
		private function onenterframe(evt:Event) {
			...
			_progressbar.width=_progress*4.65;
			_progressbutton.x=85+_playprogress*4.39;
			_playprogressbar.width=_progressbutton.x-60;
			//*****************************************************************************************************************
			_rectangle1=new Rectangle(_progressbackground.x,_progressbackground.y,_progressbar.width-26,0);
			//****************************************音频频谱*****************************************************************
			var _array:ByteArray=new ByteArray();
			SoundMixer.computeSpectrum(_array);
			_bitmapdata.fillRect(_bitmapdata.rect,0x00000000);
			for (var i:int=0; i<256; i++) {
				_bitmapdata.setPixel32(i,20+_array.readFloat()*40,0xffff6600);
			}
			for (var j:int=0; j<256; j++) {
				_bitmapdata.setPixel32(j,40+_array.readFloat()*40,0xffff6600);
			}
			//*****************************************************************************************************************
			//******************************************************时间文本***************************************************
			var second:String;
		...
		private function onclick(evt:MouseEvent) {
			progressbarwidth=mouseX-85;
			_position=(progressbarwidth/465)*totaltime*1000;
			_channel.stop();
			_channel=_sound.play(_position);
		}
		private function onclick1(evt:MouseEvent) {
			progressbarwidth=mouseX-80;
			_position=(progressbarwidth/465)*totaltime*1000;
			_channel.stop();
			_channel=_sound.play(_position);
		}
		private function onmousedown(evt:MouseEvent) {
			_soundbutton.soundrect.startDrag(false,new Rectangle(10.1,-51.6,0,42));
			stage.addEventListener(MouseEvent.MOUSE_UP,onmouseup);
		}
		private function onmouseup(evt:MouseEvent) {
			_soundbutton.soundrect.stopDrag();
			_soundbutton.soundbar.height=_soundbutton.soundrect.y*(-1)-5.5;
			_volume=2.17*_soundbutton.soundbar.height/100-0.08;
		}
		//***************************************处理ID3乱码**********************************************
